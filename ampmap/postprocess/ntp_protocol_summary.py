...
if len(sys.argv) == 2:
    ...
    print(f"wrong number of args: {sys.argv}")
...
proto_alias[f"ntp-private-5-{budget}"] = {"ntpprivate-5": f"query_searchout_NTPPrivate_5_{budget}"}

proto_alias[f"ntp-private-10-{budget}"] = {"ntpprivate-10": f"query_searchout_NTPPrivate_10_{budget}"}

proto_alias[f"ntp-private-15-{budget}"] = {"ntpprivate-15": f"query_searchout_NTPPrivate_15_{budget}"}

proto_alias[f"ntp-private-20-{budget}"] = {"ntpprivate-20": f"query_searchout_NTPPrivate_20_{budget}"}

proto_alias[f"ntp-private-30-{budget}"] = {"ntpprivate-30": f"query_searchout_NTPPrivate_30_{budget}"}

proto_agg_list = [f"ntp-private-5-{budget}", f"ntp-private-10-{budget}", f"ntp-private-15-{budget}", f"ntp-private-20-{budget}", f"ntp-private-30-{budget}"]
...
ax.set_ylabel(f"Max Amplification Factor with Budget {budget}", fontsize=25)
ax.set_xlabel("Threshold", fontsize=25, labelpad=5)

#ax.set_xlabel("Query Patterns (QP) ranked by {}".format(rank_by), fontsize=25, labelpad=20)
plt.xticks(np.arange(0, 5), [5, 10, 15, 20, 30])
...
outfile = os.path.join(fig_dir, f"Fig9_max_AF_{budget}_ntp_protocols.png" )
...
fig_name = os.path.join( fig_dir , f"Fig10_AF_{budget}_ntp_Distribution.pdf")
